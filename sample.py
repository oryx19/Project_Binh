# Lưu ý: Để dùng model nhớ cài đặt pandas
# Nếu chưa thì: pip install pandas
# Lệnh imp_mauthu dùng để nhập mẫu thử 
...
class menu:
    ...
    def timbomon(self, bomon):
        ...
        while cur_node: 
            ...
            cur_node = cur_node.next
        if dem == 0:
            print( f"Danh sach khong co bo mon {bomon}" )
    ...
    def printdanhsach(self):
        ...
        while cur_node.next != None:
            dem += 1
            cur_node = cur_node.next 
            print(dem,cur_node.mahv, cur_node.hoten, cur_node.namsinh, cur_node.sex, cur_node.bomon,"So thang no:", cur_node.so_thang_no, "Nam tham gia:", cur_node.namthamgia)
...
def main():
    ...
    while True:
        ...
        #mau thu
            ...
            print()
            add = 'https://drive.google.com/file/d/18jf9idVE6sQfd6hDdlY3xlOPksK2cInS/view?usp=sharing'
            import pandas as pd
            data = pd.read_csv(add)
            for index, row in data.iterrows():
                mahv = row['MaHV']
                hoten = row['Ho ten']
                namsinh = row['Nam sinh']
                sex = row['Gioi tinh']
                bomon = row['Bo mon']
                namthamgia = row['Nam tham gia']
                so_thang_no = row['So thang no']
                ds.them(mahv, hoten, namsinh, sex, bomon, so_thang_no, namthamgia)
            ds.printdanhsach()
            print()
            print("**DA THEM HOI VIEN TEST**")
            continue
